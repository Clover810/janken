...
function textload() {
  ...
  document.getElementById("Poweruplvl").innerHTML = "Lv." + power;
  document.getElementById("Powerupcost").innerHTML = "コスト: " + cost[0] + " Score";
  document.getElementById("Robotlvl").innerHTML = "Lv." + robot;
  document.getElementById("Robotcost").innerHTML = "コスト: " + cost[1] + " Score";
}
  ...
  if (shopselector == 0) {
    ...
    if (a == true) {
      if (score >= cost[0]) {
        score -= cost[0];
        power += 1;
        cost[0] = Math.floor(1.15**power*cost[0]);
        alert ("購入成功\nパワー: " + (power-1) + "→" + power);
      } else {
        alert ((cost[0] - score) + " スコア足りません");
      }
    }
    ...
    if (a == true) {
      if (score >= cost[1]) {
        score -= cost[1];
        robot += 1;
        cost[1] = Math.floor(1.15**power*cost[1]);
        alert ("購入成功\nロボット: " + (robot-1) + "→" + robot);
      } else {
        alert ((cost[1] - score) + " スコア足りません");
      }
    }
  }
}
